    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(handle_connection(first, Arc::clone(&reply)));
    let handle2 = tokio::spawn(handle_connection(second, reply));
    let (outcome1, outcome2) = tokio::join!(handle1, handle2);
    outcome1.unwrap();
    outcome2.unwrap();
}

async fn handle_connection<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display + Send + Sync + 'static,
{
    loop {
